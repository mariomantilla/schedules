...
const Auth = () => {
    ...
    const handleLogin = async (type) => {
        ...
        if (error) {
            ...
            setHelperText({
                error: false,
                text: "Te hemos enviado un email para verificar tu dirección!",
            });
        }
    };
    ...
    const forgotPassword = async (e) => {
        ...
        if (email === null || email === "") {
            setHelperText({ error: true, text: "Tienes que introducir un email." });
        } else {
    ...
    return (
        ...
        >
            ...
            <input
                className={"bg-gray-100 border py-1 px-3"}
                type={"password"}
                onKeyUp={(e) => e.key === "Enter" && handleLogin("LOGIN")}
                name={"password"}
                ref={passwordRef}
                required
            />
